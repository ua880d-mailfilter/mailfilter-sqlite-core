...
const string* Header :: log_id (void) const
{ return &msg_log_id; }

void Header :: set_log_id (const string& tid)
{ msg_log_id = tid; }

int Header :: next_entry_ordinal (void)
{
  msg_entry_ordinal++;
  return msg_entry_ordinal;
}
...
void Header :: add_entry (const char* tag, const char* body)
{
  ...
  if (Preferences :: Instance ().use_headers_sqlite3 ()
    ...
    {
      Dblog :: Instance ().log_header_entry (this,
                                            tag,
                                            body,
                                            next_entry_ordinal ());
    }
